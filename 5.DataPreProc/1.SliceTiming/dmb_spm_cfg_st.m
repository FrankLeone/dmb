...
data.tag     = 'data';
data.name    = 'Data';
data.help    = {'Select images to slice-time correct.'};
data.filter = 'image';
data.ufilter = '.*';
data.num     = [2 Inf];
...
targetDir.name    = 'Target directory slice-timed files';
targetDir.help    = {'Where do you want to move the slice-timed files to, if anywhere?'};
...
st.val     = {data nslices tr ta so refslice prefix expected_n_sessions targetDir};
st.help    = {
...
dep            = cfg_dep;
dep.sname      = sprintf('Slice Timing Corr. Images for all sessions');
